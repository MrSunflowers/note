...
在 innoDB 存储引擎中，每行数据都存储在页中，为了对页进行区分，那就需要引入**页号**（其实是一个表空间的地址偏移量）。同时为了把这些数据页给关联起来，于是引入了前后指针，用于指向前后的页。这些都被加到了页头里。
...
![image-20250115204158796](https://raw.githubusercontent.com/MrSunflowers/images/main/note/images/202501152042980.png)
...
![image-20250115204316913](https://raw.githubusercontent.com/MrSunflowers/images/main/note/images/202501152043090.png)
...
![image-20250115204355700](https://raw.githubusercontent.com/MrSunflowers/images/main/note/images/202501152043862.png)
...
![image-20250115204430176](https://raw.githubusercontent.com/MrSunflowers/images/main/note/images/202501152044340.png)
...
![image-20250115204549004](https://raw.githubusercontent.com/MrSunflowers/images/main/note/images/202501152045189.png)
...
![image-20250115204628279](https://raw.githubusercontent.com/MrSunflowers/images/main/note/images/202501152046448.png)
...
![image-20250115215108626](https://raw.githubusercontent.com/MrSunflowers/images/main/note/images/202501152151801.png)
...
全同步复制是指主服务器在提交事务之前，必须确保所有从服务器都成功接收到并应用了该事务。只有在所有从服务器都确认事务完成后，主服务器才会向客户端返回事务提交成功的响应。

### 半同步复制

半同步复制是介于全同步复制和异步复制之间的一种机制。主服务器在提交事务时，至少有一个从服务器确认接收到事务后，主服务器才会向客户端返回事务提交成功的响应。

## MySQL 高可用 MHA 架构方案

MySQL 高可用 MHA 架构方案最早由日本提出，基于 MySQL 的半同步复制，保证集群在主机宕机的情况下至少存在一个与主节点数据完全同步的节点，并通过第三方中间件选出该节点并提升为主节点。但其不保证从节点的运行状态，需要自行实现从属节点的健康性。优点是集群数据延迟较低。

https://zhuanlan.zhihu.com/p/132508138

## MySQL 高可用 MGR 架构

MySQL 高可用 MGR 架构基于全同步复制，利用集群一致性选举方案来选定主节点，优点是集群稳定性强，缺点是数据延迟较高
