# 为什么 MySQL 单表数据不建议超过 2000W 聚集索引 B＋ 树与 B树 的检索原理与区别
...
EverDB是一种基于MySQL的分布式数据库系统，旨在提供高可用性、可扩展性和高性能的数据库解决方案。

### 1. 架构概述

EverDB的架构基于分布式系统，采用主从复制和分片技术来实现数据的高可靠性和高性能。其核心组件包括：

- **EDB-Grid**：这是EverDB的最核心组件，负责全局调度、全局事务管理、分布式执行计划的生成与调度、集群扩缩容以及数据节点的故障自动切换。
- **数据节点**：基于MySQL社区版本，支持MySQL主从架构和MGR（MySQL Group Replication）架构，负责数据的存储、本地事务管理和本地结果集计算。
- **配置节点**：基于Zookeeper实现，负责运行态的元数据存储、同步与配置管理。
- **EDB-Control**：作为仪表盘和运维管理控制台，负责EverDB的全生命周期管理。
- **EDB-Bridge**：数据同步工具，负责将EverDB的数据增量同步给其他异构数据库。

### 2. 技术特性

#### 2.1 数据分片（Sharding）

EverDB通过数据分片技术将数据分散到多个节点上，以提高系统的扩展性和性能。分片主要针对表进行横向拆分，支持以下几种分片算法：

- **Hash分片**：通过哈希函数将数据分散到不同的节点上。
- **Mod分片**：基于取模运算进行分片。
- **List分片**：根据列表中的值进行分片。
- **Range分片**：根据范围进行分片。
...
EverDB在接收到SQL语句后，会进行语法解析并生成分布式执行计划。这个过程中会遵循一些优化原则，如查询计算下推、跨节点并行计算和必要的SQL改写，以提高执行效率。

#### 2.3 通信协议

EverDB兼容MySQL通信协议，并支持大部分常用的MySQL语法和功能。这意味着现有的MySQL客户端和应用程序可以无缝地与EverDB进行通信。

#### 2.4 分布式事务

EverDB支持分布式事务，采用XA事务协议和两阶段提交（2PC）方法来实现。这确保了事务的原子性、一致性、隔离性和持久性。

#### 2.5 数据库安全

EverDB在数据库安全方面提供了多层次的保护措施，包括：

- **访问安全**：支持账户密码访问、设置黑白名单、数据库表授权等。
- **安全审计**：可以针对操作类型、操作对象和操作用户进行安全审计。
- **数据安全**：通过数据加密和多副本多机房部署来增强数据安全性。

### 3. 高可用性

EverDB通过以下方式实现高可用性：

- **双机房部署**：在不同的物理位置部署多个节点，以防止单点故障。
- **全组件冗余设计**：所有关键组件都有冗余备份，确保在故障时能够快速切换。
- **故障自动感知和自动切换**：使用类raft协议实现主节点的故障检测、leader选举和自动故障切换。

### 4. 扩展性

EverDB支持按需在线扩展，可以通过增加数据节点来提升并发承载能力、存储容量和IOPS能力。此外，EverDB还支持在线滚动升级，确保业务连续性。

# MySQL InnoDB 缓冲池 buffer pool

实际上，InnoDB 存储引擎自引入以来就包含了缓冲池的概念，并且在 MySQL 5.x 版本中就已经存在。缓冲池是 InnoDB 存储引擎的核心组件之一，用于缓存数据和索引页，以提高数据库的性能。

1. **早期版本（MySQL 5.x 及之前）**：
   - InnoDB 缓冲池在早期版本中已经存在，并且是 InnoDB 存储引擎的重要组成部分。
   - 在这些版本中，缓冲池的基本功能已经具备，包括缓存数据页和索引页、使用 LRU 算法管理缓冲池中的页等。

2. **MySQL 5.5 及之后版本**：
   - InnoDB 成为 MySQL 的默认存储引擎，缓冲池的功能得到了进一步增强和优化。
   - 引入了多缓冲池实例（Multiple Buffer Pool Instances），以提高多核处理器的并发性能。

3. **MySQL 8.x 及之后版本**：
   - 缓冲池的功能得到了更全面的改进和优化，包括更精细的配置选项、更高效的内存管理和更强大的监控工具。
   - 引入了缓冲池持久化（Buffer Pool Persistence）功能，可以在数据库重启后快速恢复缓冲池状态。
   - 增加了对更大缓冲池大小的支持，以及更灵活的内存分配策略。

- **多缓冲池实例**：在 MySQL 5.5 及之后版本中引入，允许将缓冲池划分为多个独立的实例，以提高并发性能。
- **缓冲池预热**：在 MySQL 8.x 中得到了更好的支持，可以通过配置参数在数据库启动时自动加载缓冲池内容。
- **缓冲池持久化**：MySQL 8.x 引入了更完善的缓冲池持久化机制，可以在重启后快速恢复缓冲池状态。
- **更精细的配置选项**：MySQL 8.x 提供了更多的配置参数，允许用户更灵活地调整缓冲池的行为和性能。

缓冲池的主要作用是缓存 InnoDB 表的数据和索引。通过将频繁访问的数据和索引页缓存在内存中，可以减少磁盘 I/O 操作，从而提高查询和事务处理的性能。

## 存储结构

缓冲池由多个缓冲池实例（Buffer Pool Instances）组成，每个实例都是一个独立的内存区域。这种设计有助于提高并发性能，尤其是在多核处理器上。每个缓冲池实例包含多个页（Pages），每个页的大小通常为 16KB。

缓冲池由下列数据结构组成

- **数据页（Data Pages）**：存储表的数据。
- **索引页（Index Pages）**：存储索引数据。
- **自适应哈希索引（Adaptive Hash Index）**：InnoDB 内部使用的一种哈希索引，用于加速某些查询。
- **锁信息（Lock Information）**：存储行级锁的信息。
- **脏页（Dirty Pages）**：已被修改但尚未写入磁盘的数据页。

## 参数配置

在 MySQL 8.x 中，可以通过配置参数 `innodb_buffer_pool_size` 来设置缓冲池的大小。建议将缓冲池的大小设置为服务器内存的 50% 到 80%，具体取决于服务器的工作负载和其他内存需求。在可以保证服务器正常运行的情况下，缓冲池的大小要尽可能大。

```sql
SET GLOBAL innodb_buffer_pool_size = 8G;
```

- **调整缓冲池大小**：根据工作负载和服务器内存调整 `innodb_buffer_pool_size`。
- **增加缓冲池实例数**：对于多核处理器，可以增加 `innodb_buffer_pool_instances` 的数量，以提高并发性能。
- **监控缓冲池命中率**：缓冲池命中率越高，说明缓存效果越好。可以通过 `SHOW GLOBAL STATUS LIKE 'Innodb_buffer_pool_%';` 命令查看缓冲池命中率。

## 缓冲池预热

缓冲池预热（Buffer Pool Warmup）是指在数据库启动后，将之前缓存的数据页重新加载到缓冲池中，以减少冷启动时的性能下降。MySQL 8.x 引入了 `innodb_buffer_pool_dump_at_shutdown` 和 `innodb_buffer_pool_load_at_startup` 参数，用于在关闭和启动时自动转储和加载缓冲池内容。
...
InnoDB 使用一种称为“最近最少使用”（LRU，Least Recently Used）的算法来管理缓冲池中的页。LRU 算法将缓冲池分为两部分：年轻区和年老区。新访问的页首先进入年轻区，如果频繁访问，则会移动到年老区。脏页会在后台线程的调度下定期刷新到磁盘。
...
该命令会显示缓冲池的详细信息，包括缓冲池的使用情况、脏页的数量、LRU 列表的状态等。

### 1. **缓冲池的分区结构**

#### 年轻区（New Sublist）
- **位置**：缓冲池的前半部分（默认情况下，占缓冲池的 3/8）。
- **作用**：存储最近被访问的页。
- **特点**：
  - 新访问的页首先被插入到年轻区的头部。
  - 年轻区中的页被认为是“热”数据，因为它们最近被使用过。

#### 年老区（Old Sublist）
- **位置**：缓冲池的后半部分（默认情况下，占缓冲池的 5/8）。
- **作用**：存储较少被访问的页。
- **特点**：
  - 年老区中的页被认为是“冷”数据，因为它们最近没有被频繁访问。
  - 当缓冲池需要腾出空间时，年老区中的页会首先被淘汰。

### 2. **页的插入和移动**

#### 新页的插入
- 当一个新的数据页被访问时，它首先被插入到年轻区的头部。
- 这种策略确保了最近被访问的页位于年轻区的头部，从而提高了这些页在后续访问中被快速找到的概率。

#### 页的晋升
- 如果一个页在年轻区中被频繁访问，它会被移动到年老区的头部。
- 这种晋升机制确保了那些被频繁访问的页能够长期驻留在缓冲池中，避免被过早淘汰。

### 3. **LRU 算法的优化策略**

#### 预读（Read-Ahead）
- InnoDB 实现了预读机制，即在读取一个页时，会预先读取相邻的页，以利用磁盘的顺序读取特性。
- 预读的页会被插入到年老区的尾部，而不是年轻区。这是为了避免预读页占用年轻区的空间，从而影响真正热数据的缓存。

#### 全表扫描（Full Table Scan）
- 在进行全表扫描时，访问的页会被插入到年老区的尾部，而不是年轻区。
- 这是因为全表扫描通常是一次性的大规模数据访问，不会重复访问这些页。将它们插入到年老区可以避免污染年轻区，从而保护真正热的数据。

### 4. **页的淘汰机制**

- 当缓冲池需要腾出空间时，淘汰机制会从年老区的尾部开始淘汰页。
- 这种策略确保了那些最近没有被频繁访问的页首先被淘汰，从而为新的热数据腾出空间。

### 5. **脏页的处理**

- **脏页（Dirty Pages）**：被修改但尚未写入磁盘的数据页。
- InnoDB 使用后台线程（Page Cleaner Thread）定期将脏页刷新到磁盘。
- 刷新后的脏页会被标记为干净页，并按照 LRU 算法的规则进行管理。
- 如果缓冲池空间不足，脏页也会被优先刷新，以确保有足够的干净页可供淘汰。

### 6. **参数调优**

#### `innodb_lru_scan_depth`
- 该参数控制 LRU 算法扫描年老区的深度，以确定需要刷新的脏页数量。
- 较大的值可以提高刷新效率，但会增加 CPU 和 I/O 开销。

#### `innodb_old_blocks_pct`
- 该参数控制年老区在缓冲池中的比例。
- 默认值为 37（表示 37%），即缓冲池的后半部分为年老区。

#### `innodb_old_blocks_time`
- 该参数控制一个页在年老区中停留的最小时间（以毫秒为单位），以防止短期频繁访问的页被过早晋升到年轻区。
- 默认值为 1000 毫秒。
...
2. **频繁访问**：
   - 如果该页在短时间内被多次访问，它会被移动到年老区的头部。
   - 经过一段时间的频繁访问后，它会被晋升到年轻区。
...
MySQL 8.x 引入了缓冲池持久化（Buffer Pool Persistence）功能，可以在数据库重启后快速恢复缓冲池的状态。通过配置 `innodb_buffer_pool_dump_pct` 参数，可以控制转储缓冲池内容的比例。
