...
### 10.简要说明一下drop，delete和truncate的区别
   ...
   1）都表示删除，只是drop是指在表结构上的操作，而 delete和truncate是对表的数据的操作；

   2）truncate是指完全的删除表的数据和delete不带筛选条件的效果一样，在速度上drop>truncate>delete；

   3）对于delete由于是dml,所以在删除前会先将操作防止到rollback中，可以进行回滚，只有提交事务命令才会生效，而truncate和drop不会进行回滚。
