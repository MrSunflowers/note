...
能够选择这种模型的底气在于万一客户端拿到了已经发生变动的错误地址，也能够通过 Ribbon 和 Hystrix 模块配合来兜底，实现故障转移（Failover）或者快速失败（Failfast）
...
Nginx + Keepalived 3 台 处理静态资源和 SSL 终端 
...
在每个事物的参与者服务中都记录一张交易的流水记录表，一方面来保证交易的幂等性，另一方面在事物过程中如果失败，可以发送消息通知，根据交易流水表来进行反向补偿。
