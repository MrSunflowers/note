...
- 异步调用：项目中存在短时间内出现大量耗费内存和时间的请求，比如生成报表文件和发送告警邮件，这些功能如果在短时间内大量实时处理，那么服务器可能会无法承受导致宕机，而这些功能实时性要求不高，就考虑使用消息中间件来异步处理
- 分布式事物：因为系统涉及的数据源很多，不同的数据源之间的事物状态需要依靠消息中间件来保证，一个具体的场景就是，将不同数据源的数据加载至ES中，业务人员在前端查看和修改，后台通过 MQ 同步给不同数据源。
-
-
- 具体来说就是在发起一笔交易的时候，由发起方生成一个全局递增且唯一的事物 ID ，每个涉及到的系统都会有一张交易流水表和当前交易在本系统中的状态，在正向调用时由微服务按顺序
