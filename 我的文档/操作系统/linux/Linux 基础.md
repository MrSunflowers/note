...
一台设备如何知道一个给定的IP地址中，哪部分是网络号，哪部分是主机号？

子网掩码用于定义定义地址的层次结构，是一个与IP地址成对出现的、同样32 位的二进制数。它的唯一作用就是明确指明 IP 地址中网络部分的位数。
...
这表示该设备处于 192.168.1.0 的网络中，在这个网络中该主机的位置为 100。


IPv4 地址的组成：

- **网络前缀**：网络前缀（Network Prefix）是 IP 地址的前 16 位，它标识了设备所在的网络。
- **主机标识符**：主机标识符（Host Identifier）是 IP 地址的后 32 位，它标识了设备在网络中的位置。
- **子网掩码**：子网掩码（Subnet Mask）是网络前缀的补码，用于标识设备所在子网的大小。
- **广播地址**：广播地址（Broadcast Address）是子网掩码的补码，用于标识子网内的所有设备。
- **子网掩码位数**：子网掩码位数（Subnet Mask Bits）是子网掩码的二进制位数，它标识了设备所在子网的大小。
...
- **IP地址协议**: 网络层逻辑地址（IPv4/IPv6）
- **MAC地址协议**: 链路层物理地址（如00-23-5A-15-99-42）
...
在现有的计算机网络体系中，虽然 IPV6 协议已经问世多年，但其与 IPV4 协议完全不兼容，这就导致其普及率一直不高，IPV4 协议对互联网的格局与影响异常巨大，可以说整个网络体系都是基于 IPV4 协议构建的。

IPv4 地址：32位，通常表示为 4 个十进制数（如 192.168.1.1），总量约 43 亿个。如今各种各样的计算机设备层出不穷，全球 IPv4 地址早在 2011 年就已分配完毕。所有新加入互联网的设备、网站和服务都无法再获得公网IPv4 地址。继续依赖 IPv4 就像在土地有限的岛上盖房，只能不断地加盖违建（NAT 网络地址转换协议）来勉强维持。
...
如此即可节省公网 IP：使成百上千的设备可以共享一个公网IPv4 地址上网。
...
网段是网络中的逻辑分组，通常由一个子网掩码和子网地址组成。不同网段间是无法通信的。原因是

在不同的网络段之间进行数据传输，需要使用网关。

网关是连接不同网段的通讯设备，允许不同网络之间进行数据传输。
