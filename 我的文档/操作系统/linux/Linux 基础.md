...
计算机间进行信息传输是通过计算机网络传输的，而计算机网络是基于多种传输协议组成的，先来介绍网络分层模型

- 应用层：应用层协议（如 HTTP, FTP, SMTP, Telnet, DNS）
- 传输层：传输层协议（如 TCP, UDP）
- 网络层：网络层协议（如 IP, ICMP, ARP）
- 数据链路层：数据链路层协议（如 PPP, Ethernet, FDDI, Token Ring）
- 物理层：物理层将数据编码为光电信号进行传输。
